package com.acme.iot.platform.billing.infrastructure.persistence.jpa.repositories;

import com.acme.iot.platform.billing.infrastructure.persistence.jpa.entities.PlanFeaturePersistenceEntity;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.stereotype.Repository;

import java.util.List;

@Repository
public interface PlanFeaturePersistenceRepository extends JpaRepository<PlanFeaturePersistenceEntity, Long> {

    List<PlanFeaturePersistenceEntity> findAllByPlanId(Long planId);
}
